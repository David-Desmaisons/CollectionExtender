...
namespace MoreCollection.Set
{
    ...
    public class HashedSet<T> : ISerializable, IDeserializationCallback, IClonableSet<T>, IReadOnlyCollection<T>
    {
        ...
        public HashedSet() : this(EqualityComparer<T>.Default) { }
        ...
        public HashedSet(HashedSet<T> hashSet)
        {
            if (hashSet == null)
            {
                throw new ArgumentNullException(nameof(hashSet));
            }
            m_comparer = hashSet.m_comparer;
            m_lastIndex = hashSet.m_lastIndex;
            m_count = hashSet.m_count;
            m_freeList = hashSet.m_freeList;
            m_version = hashSet.m_version;

            Duplicate(hashSet.m_buckets, out m_buckets);
            Duplicate(hashSet.m_slots, out m_slots);  
        }

        private static void Duplicate<TElement>(TElement[] source, out TElement[] target)
        {
            if (source == null)
            {
                target = null;
        ...
        #region IClonableSet<T>

        public IClonableSet<T> Clone()
        {
            return new HashedSet<T>(this);
        }
        ...
        public void IntersectWith(IEnumerable<T> other)
        {
            ...
            if (otherAsCollection != null)
            {
                ...
                HashedSet<T> otherAsSet = other as HashedSet<T>;
        ...
        public void CopyTo(T[] array, int arrayIndex, int count)
        {
            ...
            if (arrayIndex < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(arrayIndex), "ArgumentOutOfRange Need Non Neg Num");
            }

            // also throw if count less than 0
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(count), "ArgumentOutOfRange Need Non Neg Num");
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static IEqualityComparer<HashedSet<T>> CreateSetComparer()
        {
            return new HashedSetEqualityComparer<T>();
        }
#endif
        ...
        private void IncreaseCapacity()
        {
            ...
            if (newSize <= m_count)
            {
                throw new ArgumentException("Arg HSCapacityOverflow");
            }
        ...
        /// <param name="other"></param>
        /// <returns></returns>
        private bool IsSubsetOfHashSetWithSameEC(HashedSet<T> other)
        {
        ...
        /// </summary>
        /// <param name="other"></param>
        private void IntersectWithHashSetWithSameEC(HashedSet<T> other)
        {
            for (int i = 0; i < m_lastIndex; i++)
            {
                if (m_slots[i].hashCode >= 0)
        ...
        /// </summary>
        /// <param name="other"></param>
        private void SymmetricExceptWithUniqueHashSet(HashedSet<T> other)
        {
            foreach (T item in other)
            {
                if (!Remove(item))
        ...
        /// <summary>
        /// Internal method used for HashedSetEqualityComparer. Compares set1 and set2 according 
        ...
        /// <param name="comparer"></param>
        /// <returns></returns>
        internal static bool HashSetEquals(HashedSet<T> set1, HashedSet<T> set2, IEqualityComparer<T> comparer)
        {
        ...
        /// <param name="set2"></param>
        /// <returns></returns>
        private static bool AreEqualityComparersEqual(HashedSet<T> set1, HashedSet<T> set2)
        {
        ...
        {
            private HashedSet<T> set;
            private int index;
            private int version;
            private T current;

            internal Enumerator(HashedSet<T> set)
            {
                this.set = set;
                index = 0;
            ...
            public bool MoveNext()
            {
                ...
                {
                    throw new InvalidOperationException("InvalidOperation Enum FailedVersion");
                }
            ...
            Object System.Collections.IEnumerator.Current
            {
                get
                {
                    ...
                    {
                        throw new InvalidOperationException("InvalidOperation Enum Op Cant Happen");
                    }
                    return Current;
                }
            }
            ...
            void System.Collections.IEnumerator.Reset()
            {
                if (version != set.m_version)
                {
                    throw new InvalidOperationException("InvalidOperation Enum Failed Version");
                }
